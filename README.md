...
If you want compile by yourself it is preferable to work further on the code to avoid the use of flags to force WinRT and above all the use of JUCE_MODAL_LOOP using ASync calls since modal calls can interfere with communications with the host, causing glitches or interruptions in the DSP engine.
